#import <metil_renderer.h>
#import <metil_shader_types.h>
...
- (void) loadMetalWithView: (nonnull MTKView*) metal_kit_view {
  ...
  descriptor_state_pipeline.sampleCount = metal_kit_view.sampleCount;
...
- (void) drawInMTKView: (nonnull MTKView*) metal_kit_view {
  ...
  renderPassDescriptor.colorAttachments[0].clearColor = MTLClearColorMake(
    sin((frame) / reducer) * 0.1f,
    sin((frame + 34) / reducer) * 0.25f + 0.25f,
    sin((frame + 324) / reducer) * 0.25f + 0.75f,
    0.0f
  );
  ...
  [encoder_render setFrontFacingWinding:MTLWindingCounterClockwise];
  [encoder_render setCullMode:MTLCullModeBack];
    ...
    for (
      ...
      for (
        ...
        matrix_float4x4 matrix_model_view = (matrix_float4x4) {{
          ...
          { 0, 1, 0, 0 },
          { 0, 0, 1, 0 },
          { position.x + sin((_frame + index_object) / reducer) * max_value, position.y + sin((_frame + index_object + 2348) / reducer) * max_value, position.z + sin((_frame + index_object + 5489) / reducer) * max_value, 1 }
        }};
